...
@match_router.ui("/redirect/player/{player_id}")
def redirect_to_player(player_id: str, response: Response):
    response.headers["Hx-Redirect"] = f"/player/{player_id}"
    response.status_code = 200


@match_router.page("/{match_id}")
async def view_match(match_id: str, con: Connection):
    render_match_from_id(match_id, con, redirect=redirect_to_player)


@match_router.ui("/redirect/match/{match_id}")
