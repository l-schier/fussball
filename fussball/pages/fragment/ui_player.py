...
def render_player_list(players: list[PlayerWithRating], redirect: Callable):
    with ui.element("table").classes("table table-zebra table-auto bg-base-300 overflow-scroll w-full whitespace-nowrap pr-4 pt-2 pb-2"):
        with ui.element("thead"):
            with ui.element("tr"):
        ...
        with ui.element("tbody"):
            for player in players:
                with ui.element("tr") as row:
                    row.attributes["onclick"] = f"window.location.href='/player/{player.id}'"
